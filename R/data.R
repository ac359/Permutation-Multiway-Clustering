...
#' structure eps_ij = eta_i + xi_j + u_ij and heavy-tailed (scaled t_4)
#' idiosyncratic shocks. Designed to exercise \code{\link{mwperm_dyadic}};
#' dropping the self-trade diagonal yields an incomplete array suitable for
...
#'     between the pair (true coefficient -1.0).}
#'   \item{log_gdp_i}{Numeric node-level covariate: log GDP of the importer
#'     (true coefficient 0.7); constant across exporters within an
#'     importer.}
...
#'     (true coefficient 0); useful for checking the null rejection rate.}
#' }
#' The attribute \code{"true_coef"} stores the data-generating coefficients
#' (intercept 2.0, \code{log_dist} -1.0, \code{log_gdp_i} and
#' \code{log_gdp_j} 0.7, \code{border} 0.4, \code{placebo}
#' 0).
#' @source Seeded simulation; the generator script is
#' \code{data-raw/make_data.R} in the package's source repository (not installed
#' with the package).
#' @seealso \code{\link{mwperm_dyadic}}, \code{\link{mwperm_missing}},
#'   \code{\link{mwperm}}.
#' @examples
#' data(trade_dyadic) str(trade_dyadic) attr(trade_dyadic, "true_coef")
"trade_dyadic"
...
#' condition InvB and \code{\link{mwperm_panel}}: errors are exchangeable across
