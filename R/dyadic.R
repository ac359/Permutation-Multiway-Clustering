...
#' (IPT, Procedure 1) of Guo, Toulis and Wang (2026); see
#' \code{\link{build_perm_set}} for the permutation construction.
#'
...
#' so with \code{n_reps = 1} the p-value is exact as stated.
#'
#' With \code{n_reps > 1} the reported p-value is the median of the
#' per-repetition p-values, following Guo, Toulis and Wang (2026, Remark 1),
#' and the confidence set is the set of null values that same median does not
...
#'
#' When the guarantee must hold as stated for \code{n_reps > 1}, use
#' \code{aggregate = "median2"}: twice the median, capped at 1, is a valid
#' p-value at level \code{alpha} under arbitrary dependence across
#' repetitions, and its confidence set is correspondingly wider.
#'
...
#' two defining conditions on the projection actually ask for.
#'
#' Exactly collinear nuisance columns are therefore handled exactly. A
#' \emph{nearly} collinear column of \code{x} (relative tolerance about
#' \code{1e-7} on the singular values) is dropped from the projection -- the
#' fit then behaves as if that column had been deleted, which changes what is
#' partialled out; prefer non-redundant nuisance covariates. If \code{d} is
#' annihilated entirely by some permutation's projection, the fit stops rather
#' than returning a statistic decided by rounding noise.
#'
...
#' resolution}: the smallest p-value the test can return is
#' \code{1 / (K + 1)}, so the largest admissible group is what makes rejection
#' at a small \code{alpha} possible at all (a 95\% confidence set needs
#' \code{K + 1 >= 20}). It also means \code{K + 1} equals the number of
...
#' relabelled index set with no fixed tail. The \emph{power} result, Theorem 2
#' of Guo, Toulis and Wang (2026), is stated for a \emph{fixed} \code{K} with
#' the number of clusters divisible by \code{K + 1}, i.e. many blocks of fixed
#' size as the design grows -- a different regime. Validity is unaffected
#' either way (Theorem 1 holds for any group Algorithm 1 builds), so the
#' default stands: it is the arrangement that gives the finest grid, and the
#' theoretical power statement simply does not describe it. Set \code{K}
...
#' workers on Unix and a PSOCK cluster on Windows. Only the seeded repetition
#' axis parallelises well (near-ideal speedup for large problems); the
#' per-permutation fallback axis is at best break-even, so with
#' \code{n_reps = 1} or \code{seed = NULL} expect little or no gain.
#'
...
#' coefficient) or \code{conf_region}/\code{conf_box} (several coefficients:
#' the retained beta vectors and their marginal extent) hold the \emph{IPT}
...
#' = log_dist, x = cbind(log_gdp_i, log_gdp_j), row = importer, col =
#' exporter, seed = 1)) fit
#' @export
