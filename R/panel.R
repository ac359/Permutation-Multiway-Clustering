...
#'
#' where i, j index two cross-sectional clustering dimensions (e.g. importer
#' and exporter countries) and t indexes time. Full three-way exchangeability
#' is implausible because errors are typically autocorrelated over time.
#' Instead this test only assumes exchangeability across the first two
#' dimensions *within* each time period (condition InvB of Guo, Toulis and
#' Wang, 2026),
...
#'
#' which holds, for instance, under eps_ijt = eta_i + xi_j + zeta_t + u_ijt
#' with an *arbitrary* time trend zeta_t. The same row/column permutation is
...
#' exactly once.
#'
#' A feasibility note for long panels: the engine requires `N > 2p` (with p
#' the number of nuisance columns including the intercept and, with `time_fe =
#' TRUE`, the time dummies). This matches the premise of the validity theorem
#' and is conservative here. The projection is onto the orthogonal complement
#' of the column space of `[X | X_k]`, where `X_k` is the permuted copy of
#' `X`, so its dimension is `N - rank([X | X_k])`, not the `N - 2p` of the
#' paper's full-rank statement: the intercept is permutation-invariant in
#' every design, and here the time dummies are too (time is held fixed), so
#' the stack is rank-deficient by construction and the projection retains
#' *more* dimensions than `N - 2p`. That is the correct reading -- `N - 2p` is
...
#' @param x Optional nuisance covariates x_ijt; intercept added internally.
#'   May be `NULL`.
...
#' @param time_fe Logical; if `TRUE` (the default) time fixed effects are
#'   added to the nuisance design. Under condition InvB this is valid (the
#'   time dummies are invariant to the within-period permutation) and it
#'   removes the time trend zeta_t from the residuals, which de-biases the
#'   reported point estimate and sharpens the test when treatment timing is
#'   correlated with the period.
#' @param K Number of non-identity permutations; defaults to `min(n_row,
#'   n_col) - 1` capped at 199.
#'
...
#'   under multi-way clustering and missing data, Section 6.2.
#'   arXiv:2601.08610.
#'
#' @seealso [mwperm_dyadic()], [mwperm_threeway()].
#'
#' @examples
#' data(trade_panel)
