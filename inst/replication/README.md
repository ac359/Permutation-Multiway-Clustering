...
| `06_size_by_design.R` | Type-I error for the four designs `01_size.R` omits — replicated layouts, irregular layouts, incomplete arrays and incomplete panels — at `n_reps = 1`, with `K` printed beside every rate and a check that rejection at `alpha` is attainable before a rate is quoted. The irregular design is run with a cell-constant covariate under the default random per-cell trim (Section 6.4 as printed) *and* with one that varies within cells under a common period effect, which needs `trim = "levels"` (the `rep` level held fixed across cells): under the random trim that second arm rejects a true null essentially always, which is why the script names the trim explicitly. |
