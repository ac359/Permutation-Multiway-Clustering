...
| `07_size_signflip.R` | The sign-flip test (`mwperm_dyadic_het()`, IPT-Het) against the permutation test on the same heteroskedastic gravity draws (0.5.0): the permutation test's size rises with the strength of the heteroskedasticity while the sign-flip test's stays at or below nominal, and under homoskedastic errors the sign-flip test is the *less* powerful of the two -- the trade-off the man page states. |
