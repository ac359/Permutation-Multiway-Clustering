## find_bicliques() contract tests (audit Phase 5, brief §5 item 7 /
## CLAUDE.md §5.6). Base-R stopifnot style; fast.
...
blocks_ok <- function(blocks, ri, ci, min_block) {
  ...
  for (b in blocks) {
    ...
    if (length(intersect(b$rows, seen_r)) || length(intersect(b$cols, seen_c)))
      return(FALSE)
    seen_r <- c(seen_r, b$rows); seen_c <- c(seen_c, b$cols)
  }
  TRUE
}
...
set.seed(42)
for (m in 1:15) {
  nr <- sample(5:10, 1); nc <- sample(5:10, 1)
  g <- expand.grid(i = seq_len(nr), j = seq_len(nc))
  g <- g[runif(nrow(g)) < runif(1, 0.4, 0.95), ]
  if (nrow(g) < 4L) next
  mb <- sample(2:3, 1)
  for (meth in c("greedy", "exact")) {
    bl <- suppressWarnings(find_bicliques(g$i, g$j, min_block = mb, method = meth))
    stopifnot(blocks_ok(bl, g$i, g$j, mb))
  }
}

## ---- 2. determinism + RNG hygiene --------------------------------------------
g <- expand.grid(i = 1:10, j = 1:10); set.seed(5); g <- g[runif(nrow(g)) < .7, ]
...
g20 <- expand.grid(i = 1:20, j = 1:20); g20 <- g20[g20$i != g20$j, ]
w <- NA_character_
bl20 <- withCallingHandlers(
  find_bicliques(g20$i, g20$j, min_block = 3, method = "exact", node_budget = 50L),
...
stopifnot(length(find_bicliques(1:5, 1:5, min_block = 2)) == 0L)  # diagonal only
