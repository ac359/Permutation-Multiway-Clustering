...
## methods honour their documented guarantees. Each section names the defect it
## guards against, so a failure here says what regressed rather than which
## historical ticket it came from: silent coercion of a factor `y`, a
## degenerate `d` whose p-value was decided by float noise, `K`/`seed`
## validation that blamed the data instead of the argument, validation that
## skipped cells the biclique step would later discard, an NA slipping through
## layout `rep`, and `confint(parm=)` being accepted and ignored. The
## design-detection counterparts live in test-detection.R.
library(mwperm)
...
## discards cells: an NA errors whether its cell is retained...
...
## names the block as the cause; attainable alpha -> no such note
...
## minorization gives p = 1; float noise must never decide instead)
...
for (r in list(0L, 1.5, NA_integer_, c(2L, 3L)))
  expect_err(mwperm_dyadic(y6, d6, row = g6$i, col = g6$j, n_reps = r),
             "`n_reps`")
## K validation names the argument, incl. NA / vector / fractional / 0
for (k in list(NA, NA_integer_, c(3, 4), 2.5, "3", 0L))
  expect_err(mwperm_dyadic(y6, d6, row = g6$i, col = g6$j, K = k), "`K`")
## seed: type validation and a loud, `seed`-named overflow error;
...
## K = 0 blames the argument, not the data (it used to say "Not enough
...
## attribute (it was previously accepted and ignored)
...
expect_err(mwperm_layout(yl, dl, row = gl$i, col = gl$j, rep = repv[-1]),
           "`rep`")
## NA in `rep` is rejected like every other identifier (it was
