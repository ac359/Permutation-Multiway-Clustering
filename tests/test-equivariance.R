...
set.seed(1)
n <- 21L; g <- expand.grid(i = seq_len(n), j = seq_len(n)); N <- nrow(g)
x <- cbind(z = rnorm(N)); d <- rnorm(n)[g$i] + rnorm(N)
...
f2 <- mwperm_dyadic(y + (3 + 1.7 * x[, 1]), d, x = x, row = g$i, col = g$j, seed = 5)
...
stopifnot(max(abs(sort(-fs$conf_int) - f0$conf_int)) < 1e-10)  # CI negates+flips
...
gp <- expand.grid(i = 1:6, j = 1:6, t = 1:4); Np <- nrow(gp)
set.seed(2); dp <- rnorm(Np); yp <- rnorm(6)[gp$i] + rnorm(6)[gp$j] + cumsum(rnorm(4))[gp$t] + 0.3 * dp + rnorm(Np)
...
g3 <- expand.grid(a = 1:5, b = 1:5, c = 1:5); N3 <- nrow(g3)
set.seed(3); d3 <- rnorm(N3); y3 <- rnorm(5)[g3$a] + rnorm(5)[g3$b] + rnorm(5)[g3$c] + rnorm(N3)
t_dir <- mwperm_threeway(y3, d3, id1 = g3$a, id2 = g3$b, id3 = g3$c, seed = 4)
t_dis <- mwperm(y = y3, d = d3, index = list(id1 = g3$a, id2 = g3$b, id3 = g3$c),
                design = "threeway", seed = 4, verbose = FALSE)
stopifnot(isTRUE(same_fit(t_dir, t_dis, skip = c("call", "auto"))))

gl <- expand.grid(l = 1:5, i = 1:4, j = 1:4); Nl <- nrow(gl)
set.seed(4); dl <- rnorm(Nl); yl <- rnorm(16)[as.integer(interaction(gl$i, gl$j))] + 0.5 * dl + rnorm(Nl)
l_dir <- mwperm_layout(yl, dl, row = gl$i, col = gl$j, seed = 6)
l_dis <- suppressWarnings(mwperm(y = yl, d = dl, index = list(row = gl$i, col = gl$j),
                                 design = "layout", seed = 6, verbose = FALSE))
stopifnot(isTRUE(same_fit(l_dir, l_dis, skip = c("call", "auto"))))

gm <- expand.grid(i = 1:12, j = 1:12); gm <- gm[gm$i != gm$j, ]     # drop diagonal
set.seed(5); keep <- sample(nrow(gm), round(0.9 * nrow(gm))); gm <- gm[keep, ]
Nm <- nrow(gm); dm <- rnorm(Nm); ym <- rnorm(12)[gm$i] + rnorm(12)[gm$j] + rnorm(Nm)
...
f_ser <- mwperm_dyadic(y, d, x = x, row = g$i, col = g$j, seed = 5, n_reps = 15L)
f_par <- mwperm_dyadic(y, d, x = x, row = g$i, col = g$j, seed = 5, n_reps = 15L,
                       n_cores = 2L)
...
for (m in c("fork", "psock")) {
  ...
  msg <- tryCatch({ suppressWarnings(mwperm:::.plapply(
                      as.list(1:4),
                      function(i) if (i == 3L) stop("boom") else i,
                      n_cores = 2L, method = m)); NA_character_ },
...
unlockBinding(".plapply", ns); assign(".plapply", orig_plapply, envir = ns); lockBinding(".plapply", ns)
...
stopifnot(nrow(k_axis_calls) >= 3L,
          all(k_axis_calls[, "n_cores"] == 2L))                 # K axis parallel
