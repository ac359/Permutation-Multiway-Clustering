## Phase 2.1 — build_perm_set() is Algorithm 1 (GTW 2026): a random block-cyclic
## permutation group. These are EXACT invariants (machine precision / identical),
## the class of bug Monte Carlo will not reliably surface. Base-R stopifnot so it
...
cfg_nK <- function(cfg) list(n = as.integer(cfg[["n"]]), K = as.integer(cfg[["K"]]))

## ---- 1. each element is a bijection of 1:n; element 1 is the identity --------
for (cfg in list(list(n = 12, K = 3), list(n = 8, K = 7), list(n = 25, K = 4))) {
  p <- cfg_nK(cfg); n <- p$n; K <- p$K
...
for (cfg in list(list(n = 12, K = 3), list(n = 20, K = 4), list(n = 9, K = 2))) {
  p <- cfg_nK(cfg); n <- p$n; K <- p$K; B <- K + 1L
  ...
  for (k in 1:K) { it <- compose(G[[2L]], it); stopifnot(identical(it, G[[k + 1L]])) }
}

## ---- 3. remainder handling (brief H4): leftover tail is fixed, no interior FP -
## When n %% (K+1) != 0 the leftover indices cannot be cyclically shifted within a
...
err <- function(e) tryCatch({ e; NA_character_ }, error = function(x) conditionMessage(x))
...
stopifnot(identical(build_perm_set(40, 6, seed = 2), build_perm_set(40, 6, seed = 2)))
stopifnot(!identical(build_perm_set(40, 6, seed = 2), build_perm_set(40, 6, seed = 3)))
