...
## It must live on the grid {1,...,K+1}/(K+1), be conservative on ties, and equal
## 1 at each per-permutation FWL estimate u_j/M_j (the fact the island guard uses).
library(mwperm)

set.seed(3)
n <- 7L; K <- 6L; Kp1 <- K + 1L
g <- expand.grid(i = seq_len(n), j = seq_len(n)); N <- nrow(g)
X <- cbind(1, rnorm(N)); D <- matrix(rnorm(N), N, 1)
...
ps <- vapply(seq(-8, 8, by = 0.01), function(b) mwperm:::.ipt_eval(prep, b)$pvalue, numeric(1))
...
## Hand-crafted prep with an exact tie: at b = 0, a = (2,3,4), b = (2,5,6); amin = 2
...
pmin <- min(vapply(seq(-30, 30, by = 0.1), function(b) mwperm:::.ipt_eval(prep, b)$pvalue, numeric(1)))
